	...
	updateButtonStates: function (updating) {
		...
		each(rangeSelector.buttonOptions, function (rangeOptions, i) {
			var actualRange = mathRound(baseAxis.max - baseAxis.min),
				...
				// Disable the All button if we're already Mostrando all 
				isAllButAlreadyMostrandoAll = rangeOptions.type === 'all' && baseAxis.max - baseAxis.min >= dataMax - dataMin && 
					buttons[i].state !== 2,
			...
			} else if (!allButtonsEnabled && (isTooGreatRange || isTooSmallRange || isAllButAlreadyMostrandoAll || isYTDButNotAvailable)) {
